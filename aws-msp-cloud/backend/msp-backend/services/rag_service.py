...
class RAGService:
    ...
    async def search(
        ...
        Args:
            ...
            user_api_key: Optional user-provided OpenAI API key (overrides default)
        ...
        try:
            ...
            query_embedding = await self.embed_text(query, user_api_key=user_api_key)
    ...
    async def embed_text(self, text: str, user_api_key: Optional[str] = None) -> List[float]:
        """Generate embedding for text using OpenAI API"""
        # Use user-provided key if available, otherwise use default
        api_key = user_api_key or self.openai_api_key

        if not api_key:
            raise ValueError("OPENAI_API_KEY not set (provide via environment or user_api_key parameter)")
